...
class Player {
  ...
  getInput(teamSize) {
    for (let i = 0; i < teamSize; i++) {
      const playerScore = parseInt(prompt(`Enter the score of Player ${i + 1}: `));
      const team = new Team(`Player ${i + 1}`, playerScore);
...
function Scene() {
  ...
  return (
    <div>
      <h1>Match Scene</h1>
      {!widgetOpen && <button onClick={handleOpenWidget}>Open Widget</button>}
      {widgetOpen && <TeamWidget onClose={handleCloseWidget} />}
    </div>
  );
}
